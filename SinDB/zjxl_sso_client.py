...
# 直接从配置文件中的AUTH_USER_MODEL获取user
...
@login_required
def sso_log_out(request):
    logout(request)
    from django.conf import settings
    logout_url = settings.SSO_LOGOUT_URL
...
class LoginView(View):
    ...
    def get(self, request):
        ...
        path = reverse('zjxl-sso-authenticate')
...
class Client(object):
    ...
    def get_login_urls(self):
        return [
            url(r'^$', self.login_view.as_view(client=self), name='zjxl-sso-login'),
            url(r'^authenticate/$', self.authenticate_view.as_view(client=self), name='zjxl-sso-authenticate'),
        ]
